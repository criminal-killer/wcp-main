...
async function getAffiliateData(): Promise<{ affiliate: AffiliateData | null; referrals: ReferralRow[]; error?: string }> {
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'
  try {
    const [meRes, refRes] = await Promise.all([
      fetch(`${appUrl}/api/affiliates/me`, { headers: headers(), cache: 'no-store' }),
      fetch(`${appUrl}/api/affiliates/referrals`, { headers: headers(), cache: 'no-store' }),
    ])
...
export default async function AffiliateDashboard() {
  ...
  const { affiliate, referrals, error } = await getAffiliateData()
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 font-outfit">
        <div className="bg-white rounded-3xl p-10 max-w-md w-full text-center shadow-xl">
          ...
          <p className="text-slate-500 mb-6">{error || 'No affiliate account found for your profile.'}</p>
