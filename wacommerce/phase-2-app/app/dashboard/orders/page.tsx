    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">{orderList.length} orders · 300/month limit</p>
        </div>
      </div>
      ...
      {orderList.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center">
          ...
          <p className="text-gray-400 mb-6">Orders will appear here when customers buy through your WhatsApp store.</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="px-5 py-4 border-b border-gray-50 flex items-center gap-3">
            ...
            {['new', 'confirmed', 'shipped', 'delivered', 'cancelled'].map(status => (
              <button key={status} className="text-xs px-3 py-1.5 rounded-full border border-gray-200 text-gray-600 hover:border-[#25D366] hover:text-[#25D366] transition-colors capitalize">
                {status}
              </button>
            ))}
          </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {orderList.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3">
                      <p className="font-medium text-gray-900 text-sm">{order.contact_name || 'Unknown'}</p>
